...
public class AxisAlignedBB {
	...
	public double calculateXOffset(AxisAlignedBB aabb, double xOffset) {
		if(aabb.maxY > this.minY && aabb.minY < this.maxY) {
			if(aabb.maxZ > this.minZ && aabb.minZ < this.maxZ) {
				double d4;
				if(xOffset > 0.0D && aabb.maxX <= this.minX) {
				...
				if(xOffset < 0.0D && aabb.minX >= this.maxX) {
	...
	public double calculateYOffset(AxisAlignedBB aabb, double yOffset) {
		if(aabb.maxX > this.minX && aabb.minX < this.maxX) {
			if(aabb.maxZ > this.minZ && aabb.minZ < this.maxZ) {
				double d4;
				if(yOffset > 0.0D && aabb.maxY <= this.minY) {
				...
				if(yOffset < 0.0D && aabb.minY >= this.maxY) {
	...
	public double calculateZOffset(AxisAlignedBB aabb, double zOffset) {
		if(aabb.maxX > this.minX && aabb.minX < this.maxX) {
			if(aabb.maxY > this.minY && aabb.minY < this.maxY) {
				double d4;
				if(zOffset > 0.0D && aabb.maxZ <= this.minZ) {
				...
				if(zOffset < 0.0D && aabb.minZ >= this.maxZ) {
