...
public class Pathfinder {
	...
	public PathEntity createEntityPathTo(Entity entity, int x, int boundingBoxY, int z, float f5) {
		return this.createEntityPathTo(entity, (double)((float)x + 0.5F), (double)((float)boundingBoxY + 0.5F), (double)((float)z + 0.5F), f5);
	}

	private PathEntity createEntityPathTo(Entity entity, double x, double boundingBoxY, double z, float f8) {
		this.path.clearPath();
		...
		PathEntity pathEntity12 = this.addToPath(entity, pathPoint9, pathPoint10, pathPoint11, f8);
		return pathEntity12;
	}

	private PathEntity addToPath(Entity entity, PathPoint pathPoint1, PathPoint pathPoint2, PathPoint pathPoint3, float f5) {
		...
		while(!this.path.isPathEmpty()) {
			...
			int i8 = this.findPathOptions(entity, pathPoint7, pathPoint3, pathPoint2, f5);
	...
	private int findPathOptions(Entity entity, PathPoint pathPoint1, PathPoint pathPoint2, PathPoint pathPoint3, float f5) {
		int i6 = 0;
		byte b7 = 0;
		...
		if(pathPoint8 != null && !pathPoint8.isFirst && pathPoint8.distanceTo(pathPoint3) < f5) {
			this.pathOptions[i6++] = pathPoint8;
		}

		if(pathPoint9 != null && !pathPoint9.isFirst && pathPoint9.distanceTo(pathPoint3) < f5) {
			this.pathOptions[i6++] = pathPoint9;
		}

		if(pathPoint10 != null && !pathPoint10.isFirst && pathPoint10.distanceTo(pathPoint3) < f5) {
			this.pathOptions[i6++] = pathPoint10;
		}

		if(pathPoint11 != null && !pathPoint11.isFirst && pathPoint11.distanceTo(pathPoint3) < f5) {
	...
	private PathPoint getSafePoint(Entity entity, int i2, int i3, int i4, PathPoint pathPoint, int i6) {
		...
		if(this.getVerticalOffset(entity, i2, i3, i4, pathPoint) > 0) {
			pathPoint7 = this.openPoint(i2, i3, i4);
		}

		if(pathPoint7 == null && this.getVerticalOffset(entity, i2, i3 + i6, i4, pathPoint) > 0) {
		...
		if(pathPoint7 != null) {
			...
			int i10;
			for(boolean z9 = false; i3 > 0 && (i10 = this.getVerticalOffset(entity, i2, i3 - 1, i4, pathPoint)) > 0; --i3) {
				if(i10 < 0) {
					return null;
				}
